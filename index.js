...
exports.parseStream = function(stream, callback) {
  ...
  inflate.on("data", function(data) {
    ...
    for(i = 0; i !== len; ++i) {
      console.log(b + ': [' + i + ']' + ' -> ' + data[i] + ' (' + data[i].toString(16) + ')', data, data.length)
      if(b === -1) {
        ...
        console.log('scanlineIndex', scanlineIndex + ' (' + (+scanlineIndex + 1) + ')', 'scanlinePixels', scanlinePixels, 'scanlineBytes', scanlineBytes)
      ...
      if(++b === scanlineBytes) {
        ...
        var x = 0,
            y = scanlineIndex;
        for(scanlinePixelIndex = 0; scanlinePixelIndex !== scanlinePixels; ++scanlinePixelIndex) {
          /* Read all of the samples into the sample buffer. */
          for(j = 0, k = 0; k !== pngSamplesPerPixel; ++j, ++k)
            switch(pngBitDepth) {
              case 1:
                pngSamples[k] =
